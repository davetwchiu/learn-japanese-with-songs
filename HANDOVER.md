...
- Vocabulary quiz reading-clue fix（`d50b9fd`）：normal／Cloudflare builds、ESLint、
  ...
  `d50b9fd`。
- Vocabulary quiz answer-review release（`307f922`）：normal／Cloudflare builds、
  ESLint、service-worker syntax、`git diff --check` 及 27/27 tests 均通過。答錯後
  正確答案會保持顯示並要求用家自行按「下一題」，答對則繼續自動跳題。OpenAI
...
- Advanced reading quiz release（`dfe4c4e`）：normal／Cloudflare builds、ESLint、
  ...
  `dfe4c4e`。
- Player toggle release（`4a02785`）：normal／Cloudflare builds、TypeScript、
  ESLint、Node test runner 27/27、service-worker syntax、兩個 Wrangler dry-run
  及 `git diff --check` 均通過。Control bar 改為倒退 5 秒、單一播放／暫停切換
  按鈕和快進 5 秒；播放中顯示暫停，其他狀態顯示播放。橙色只在按壓時顯示，
  放開後以 0.2 秒淡出，沒有常駐 highlight。OpenAI Sites version 47／environment
